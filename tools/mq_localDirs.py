...
####module restore testin10a
module load GCCcore/9.3.0
module load DMTCP/2.6.0
module restore testin11a
...
/mnt/ufs18/home-166/f0000194/dmtcp/bin/dmtcp_coordinator --daemon --exit-on-last -p 0 --port-file $fname $@ 1>/dev/null 2>&1   # start coordinater
...
then
  ...
  /mnt/ufs18/home-166/f0000194/dmtcp/bin/dmtcp_launch -h $DMTCP_COORD_HOST -p $DMTCP_COORD_PORT --rm --ckpt-open-files {MABE_CALL} &
  ...
  then
    # start checkpointing
    /mnt/ufs18/home-166/f0000194/dmtcp/bin/dmtcp_command -h $DMTCP_COORD_HOST -p $DMTCP_COORD_PORT --ckpt-open-files --bcheckpoint
    # kill the running job after checkpointing
    /mnt/ufs18/home-166/f0000194/dmtcp/bin/dmtcp_command -h $DMTCP_COORD_HOST -p $DMTCP_COORD_PORT --quit
    # resubmit the job
  ...
  /mnt/ufs18/home-166/f0000194/dmtcp/bin/dmtcp_restart -h $DMTCP_COORD_HOST -p $DMTCP_COORD_PORT ckpt_*.dmtcp &
  ...
  then  
    ...
    /mnt/ufs18/home-166/f0000194/dmtcp/bin/dmtcp_command -h $DMTCP_COORD_HOST -p $DMTCP_COORD_PORT --ckpt-open-files -bc
...
pathToLocalTmp = '/tmp/local/' + $SLURM_JOBID + '/'  # used for HPCC runs
...
for i in range(len(combinations)):
    for rep in reps:
        ...
        if args.runHPCC or args.indefinite:
            ...
            workDir = pathToLocalTmp + userName + '/' + \
                realDisplayName + '_' + str(rep).zfill(padSizeReps) + '__' + timeNow

            # this is where data files will actually be writen (on localTmp)
            outputDir = workDir + '/' + \
            ...
            shutil.copy(executable, workDir)  # copy the executable to localTmp
            for f in cfg_files:
                shutil.copy(f, workDir)  # copy the settings files to localTmp
            for f in replacedOther_files:
                shutil.copy(f, workDir)  # copy other files to localTmp
            ...
            # if there is already a link for this rep from the local conditions directory to localTmp, remove it
            ...
            os.chdir(workDir)  # goto the work dir (on localTmp)

            slurmFileName = "slurm.sb"

            # make the qsub file on localTmp
